...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$("[id ^= groupCodeTr]").each(function(index){
			var id = $(this).attr("id");
		...
		for(var i = 0; i < idArrayList.length; i++){
			$("[id *= groupCodeTr_"+ idArrayList[i] +"]").not(":eq(0)").remove();
		}
	});
	...
	* 상세 페이지로 이동
	━━━━━━━━━━━━━━━━━━━━━━━━━━━━━*/
	function goView(GpCd, Cd){
		document.frm.group_code_id.value = GpCd;
		document.frm.code_id.value = Cd;
		document.frm.action = "<c:url value='/kdn/admin/codeView.do'/>";
		document.frm.submit(); 
	}
	...
	* 등록 페이지로 이동
	━━━━━━━━━━━━━━━━━━━━━━━━━━━━━*/
	function goWrite(){
		document.frm.action = "<c:url value='/kdn/admin/codeWrite.do'/>";
		document.frm.submit();
	}
	...
	* 검색
	━━━━━━━━━━━━━━━━━━━━━━━━━━━━━*/
	function goSearch(){
		document.frm.action = "<c:url value='/kdn/admin/codeList.do'/>";
		document.frm.submit();
	}
	...
	* 선택한 대상 삭제
	...
	function goSelectDel(){
		...
		if (confirm('선택한 대상을 삭제 하시겠습니까?')) {
			document.frm.action = "<c:url value='/kdn/admin/codeDelete.do'/>";
			document.frm.submit();
		}
	}
...
<body>
	...
	<div id="wrap">
		...
		<form id="frm" name="frm" method="post" action="<c:url value='/kdn/admin/codeList.do'/>">
			<input name="pageIndex" type="hidden"	value="<c:out value='${box.pageIndex}'/>" />
			<input name="group_code_id" type="hidden"	value="" />
			<input name="code_id" type="hidden"	value="" />
			...
			<div class="container">
				...
				<div class="top">
					<h3>공통코드</h3>
					<p class="navi">
						<strong>Home</strong> &gt; 공통코드관리 &gt; <strong class="location">공통코드</strong>
					</p>
				</div>
				<!-- // top -->
				<!-- mid -->
				<div class="mid" >
					<ul>
						<li>
							...
							<select name="searchCnd" title="검색조건선택" class="select_st ml27">
									...
									<option value="groupCodeId" <c:if test="${box.searchCnd == 'groupCodeId'}">selected="selected"</c:if> >그룹코드ID</option>
									<option value="groupCodeName" <c:if test="${box.searchCnd == 'groupCodeName'}">selected="selected"</c:if> >그룹코드명</option>
									<option value="codeId" <c:if test="${box.searchCnd == 'codeId'}">selected="selected"</c:if> >코드ID</option>
									<option value="codeName" <c:if test="${box.searchCnd == 'codeName'}">selected="selected"</c:if> >코드ID명</option>
								</select>
					...
					<table summary="공통코드 게시판입니다.">
						<caption>공통코드 게시판</caption>
						<thead>
							<tr>
						...
						<tbody>
							<c:forEach var="result" items="${list}" varStatus="status">
								<tr id = "groupCodeTr_${result.group_code_id}">
									<td colspan="6" >  
										<span style="float:left;padding-left: 10px;"><c:out value="${result.group_code_name}" /> (<c:out value="${result.group_code_id}" />)</span>  
									</td> 
								</tr>
								<tr>
									<td>
										<input type="checkbox" name="chk" value="${result.group_code_id}/${result.code_id}" class="checkbox">
									</td>
									<td> 
										<c:out value="${totCnt - (status.count + (box.pageIndex -1 ) * box.recordCountPerPage) + 1}" />
									</td>
									<td>
										<a style="color:#729ACF;text-decoration:underline;float: left;" href="javascript:goView('<c:out value="${result.group_code_id}"/>','<c:out value="${result.code_id}"/>')"><c:out value="${result.code_id}" /></a>
									</td>
									<td>
							...
							<c:if test="${fn:length(list) == 0}">
								<c:choose>
									<c:when test="${not empty box.searchWrd}">
										<tr> 
											<td colspan="6" >
												<font style="font-family:맑은 고딕;font-size:12px">검색하신 공통코드 데이터가 없습니다.</font>
											</td>
										</tr>
									</c:when>
									<c:otherwise>
										<tr> 
											<td colspan="6" >
												<font style="font-family:맑은 고딕;font-size:12px">공통코드 데이터가 없습니다.</font>
											</td>
										</tr>	
									</c:otherwise>
								</c:choose>
