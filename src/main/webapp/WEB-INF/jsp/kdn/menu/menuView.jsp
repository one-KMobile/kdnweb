...
function goModify(){
	form = document.searchForm;
	form.action = "<c:url value='/kdn/admin/menuWrite.do'/>";
	form.submit();
}
...
function goDelete(){
	...
	if (confirm('정말로 삭제 하시겠습니까?')) {
		form.action = "<c:url value='/kdn/admin/menuDelete.do'/>";
		form.submit();	
	}
}
</script>
	...
	<div id="wrap"> 
		...
		<div id="contents">
			<form name="searchForm" id="searchForm" method="post" >
				...
				<input id="chk" name="chk" type="hidden" value="<c:out value='${viewBox.menu_idx}'/>" /><!-- 삭제 처리 하기 위해 키값 셋팅 파라메터  -->
				<!-- 검색 파라메터  -->
				...
				<div class="top">
					<h3>메뉴 상세</h3>
					<p class="navi">
						<strong>Home</strong> &gt; 메뉴관리 &gt; <strong class="location">메뉴 상세</strong>
					</p>
				</div>
				<!-- // top -->
				<table width="700" border="0" cellpadding="0" cellspacing="0"	>
					<tr>
						<th class="th_top">상위메뉴ID&nbsp;&nbsp;</th>
						<td class="td_top" style="padding-left:10px"><font style="font-weight:bold;">${viewBox.top_menu_id}</font> (상위메뉴명 : ${viewBox.top_menu_id_nm})</td>
					</tr>
					<tr>
						<th width="20%" height="23" class="required_text" scope="row" nowrap>메뉴ID&nbsp;&nbsp;</th>
						<td style="padding-left:10px;font-weight:bold;">${viewBox.menu_id}</td>
					</tr>
					<tr>
						<th width="20%" height="23" class="required_text" scope="row" nowrap>메뉴명&nbsp;&nbsp;</th>
						<td style="padding-left: 10px;">${viewBox.menu_nm}</td>
					</tr>
					<tr>
						<th width="20%" height="23" class="required_text" scope="row" nowrap>설명&nbsp;&nbsp;</th>
						<td style="padding-left: 10px;">${viewBox.menu_account}</td>
					</tr>
					<tr>
						<th width="20%" height="23" class="required_text" scope="row" nowrap>프로그램 파일명&nbsp;&nbsp;</th>
						<td style="padding-left: 10px;">${viewBox.controller_method}</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td style="padding-left: 10px;" colspan="1">${viewBox.reg_id} (${viewBox.reg_date_string})</td>
					</tr>
					<c:if test="${not empty viewBox.upd_id}">
					<tr>
						<th height="23" class="required_text" scope="row" nowrap >수정자&nbsp;&nbsp;</th>
						<td style="padding-left: 10px;" colspan="1">${viewBox.upd_id} (${viewBox.upd_date_string})</td>  
					</tr>
					</c:if>
				</table>
				...
				<div style="padding: 10px 0 0 10px">
					...
					<c:choose>
						<c:when test="${viewBox.menu_id eq '000000'}">
						</c:when>
						<c:when test="${viewBox.top_menu_id eq '000000' and viewBox.top_menu_id_count > 0}">
						</c:when>
						<c:otherwise>
