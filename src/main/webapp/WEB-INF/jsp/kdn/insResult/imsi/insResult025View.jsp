	...
	<div id="wrap">
		...
		<header>  
            ...
            <ul>      
                ...
                <li id="top_106000" class="current"><a href="<c:url value='/kdn/admin/insResultImsiView.do'/>?ins_ty_cd=025" >항공등등구</a></li> 
		...
		<form id="frm" name="frm" method="post" action="<c:url value='/kdn/admin/insResultList.do'/>">
			...
			<div class="container">
				...
				<div style="float: left;width: 1033px;margin-bottom: 10px">
					<div style="margin-top: 10px;padding-left: 18px;font-weight: bold;margin-bottom: 12px;float: left;">○ 항공장애등 등구 확인점검</div>
							...
							<tbody>   
								<c:forEach var="result" items="${subList}" varStatus="status">
									<tr>
										...
										<td>
											<c:out value="${result.flck_co}" />
										</td>
										<td>
											<c:choose>
												<c:when test="${result.lightg_stadiv_cd eq '001'}"><!-- 양호  -->
													<font style="font-size: 18px;font-weight: bold;color: #32cd32;">ⓥ</font>
												</c:when>
												<c:when test="${result.lightg_stadiv_cd eq '003'}"><!-- 불량 -->  
