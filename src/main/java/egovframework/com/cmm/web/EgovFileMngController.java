...
public class EgovFileMngController {
    ...
    public String selectFileInfsForUpdate(@ModelAttribute("searchVO") FileVO fileVO, Map<String, Object> commandMap,
		...
		String atchFileId = (String)commandMap.get("param_atchFileId");
		String addFileCount = (String)commandMap.get("param_addFileCount"); //input file 생성 카운트
		...
		model.addAttribute("addFileCount", Integer.parseInt(addFileCount));
     ...
     */
    @RequestMapping("/cmm/fms/newDeleteFileInfs.do")
    public String newDeleteFileInfs(@ModelAttribute("searchVO") FileVO fileVO, HttpServletRequest request, ModelMap model) throws Exception {
	 ...
	 * @fix(<수정자명>) [yyyy.mm.dd]: [수정 내용]
     */
    @RequestMapping("/cmm/fms/delAllFileInfBox.do")
    public String delAllFileInfBox(HttpServletRequest request, ModelMap model) throws Exception {
    	Box box = BoxUtil.getBox(request);
    	try {
    		fileService.delAllFileInfBox(box);
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
		}
